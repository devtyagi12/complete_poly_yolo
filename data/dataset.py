...
def _adjust_targets_letterbox(
    ...
    """
    Remap normalised targets from the original image coordinate space into
    the letterboxed canvas coordinate space.

    Original coords are normalised to [0,1] relative to (src_w, src_h).
    After letterbox the image occupies pixels [pad_x : pad_x+new_w] and
    [pad_y : pad_y+new_h] inside a (canvas_size × canvas_size) canvas.

    new_w = round(src_w * scale),  new_h = round(src_h * scale)

    Mapping for any normalised x:
        canvas_x = (x * src_w * scale + pad_x) / canvas_size
    """
    ...
    def _rx(x):   # remap normalised-x from src to canvas
        return (x * src_w * scale + pad_x) / s

    def _ry(y):
        return (y * src_h * scale + pad_y) / s

    def _rw(w):   # widths / heights scale but don't shift
        return w * src_w * scale / s

    def _rh(h):
        return h * src_h * scale / s
